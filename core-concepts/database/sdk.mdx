...
<Note>
To modify user data, use the auth methods like `insforge.auth.setProfile()`. The database API provides read-only access to user tables.
</Note>
...
If you're currently using direct REST API calls, the SDK provides the same functionality with better developer experience:
...
const { data, error } = await insforge.database
  .from('products')
  .insert({ name: 'Widget', price: 19.99 })
  .select()
```
</CodeGroup>
